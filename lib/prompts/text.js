...
import { style, clear, strip, figures } from './util/index.js';

const DEFAULT_ERROR = 'Please Enter A Valid Value';
/**
...
export default class TextPrompt extends Prompt {
  constructor(opts={}) {
    ...
    this.initial = opts.default || '';
    this.validator = opts.validate || (() => true);
    this.value = '';
    this.cursor = Number(!!this.initial);
    this.clear = clear('');
    this.isTextPrompt = true;
    this.render();
  }
  ...
  set value(v) {
    if (!v && this.initial) {
      ...
      this.rendered = color.dim(this.transform.render(this.initial));
    } else {
  ...
  async validate() {
    let valid = await this.validator.call(undefined, this.value);
    if (typeof valid === 'string' && valid) {
      this.errorMsg = valid;
      valid = false;
      this.error = true;
    } else if (typeof valid === 'boolean') {
      this.errorMsg = valid ? '' : DEFAULT_ERROR;
      this.error = !valid;
    } else {
      this.errorMsg = '';
      this.error = false;
  ...
  deleteWord() {
    if (this.cursor === 0) return this.bell();
    const rest = this.value.slice(this.cursor);
    do {
      this.moveCursor(-1);
      if (
        this.cursor > 0 &&
    ...
    this.value = this.value.slice(0, this.cursor) + rest;
    this.red = false;
    this.render();
  }
  ...
  render() {
    ...
    let prompt = [
      ...
      this.red ? color.red(this.rendered) : (this.done ? color.cyan(this.rendered) : this.rendered)
    ].join(' ');
