...
import { style, clear, figures, lead } from './util/index.js';
...
export default class SelectPrompt extends Prompt {
  ...
  render() {
    ...
    const prompt = [
      ...
      this.done ? color.cyan(this.selection.title) : (
      ...
      body = '\n' + this.choices
        .map((v, i) => {
          ...
          if (v.disabled) {
            ...
            title = this.cursor === i ? color.cyan.bold.underline(v.title) : color.bold(v.title);
            prefix = this.cursor === i ? color.cyan(figures.pointer) : ' ';
          }
          let choice = `${prefix} ${title}`;
          if (v.hint) {
            choice += `\n${color.dim.italic(lead(v.hint, 2))}`;
          }
          return choice;
        })
        .join('\n');
