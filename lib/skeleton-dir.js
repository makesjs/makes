...
const TMP_TARBALL_FILE = '.makes-tarball.tgz';
...
async function finalFolder(target, generate) {
  ...
  const files = fs.readdirSync(target).filter(n => 
    n !== '.keep' && n !== TMP_TARBALL_FILE
  );
  if (files.length === 1) {
...
module.exports = async function(supplier, {
  ...
  const tmpFolder = tmp.dirSync({unsafeCleanup: true}).name;
  ...
  try {
    ...
    return await _useGitRepo(tmpFolder, result);
  }
    ...
    target => {
      ...
      return new Promise((resolve, reject) => {
        https.get(
          ...
          res => {
            if (res.statusCode == 200) {
              const tgzFileName = path.join(target, TMP_TARBALL_FILE);
              res.pipe(fs.createWriteStream(tgzFileName))
                .once('error', reject)
                .once('finish', () => {
                  run('tar', ['-xf', tgzFileName, '-C', target]).then(
                    resolve,
                    reject
                  );
                });
