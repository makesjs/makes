...
module.exports = async function (supplier, {
  ...
  if (!result) {
    info('Not a publically hosted git repo: ' + supplier);
    info('Maybe this is a private skeleton.');

    return finalFolder(
      _tmpFolder,
      target => {
        // Use supplier as string
        return _useGitRepo(target, supplier);
      }
    );
  }
  ...
  return finalFolder(
    ...
    target => {
      ...
      return new Promise((resolve, reject) => {
        ...
        return _useGitRepo(target, result);
      });
    }
  );
};
